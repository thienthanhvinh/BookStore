...
<?php if (!empty($dealOfWeek)) { ?>
    ...
    <?php foreach ($dealOfWeek as $product) { ?>
        <div class="deal-week bg-mainProduct shadow-mainShadow rounded-3xl flex items-center gap-6 px-4 w-[49%] h-[300px] mb-3">
            <div class="w-[45%] h-[85%]">
                <img class="lazyload w-full h-full object-cover rounded-xl" data-src="<?php echo $base_url ?>/uploads/images/<?php echo $product['product_image'] ?>" alt="" loading="lazy">
            </div>
